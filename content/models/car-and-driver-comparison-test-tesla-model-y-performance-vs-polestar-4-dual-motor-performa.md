# Model Card — Car and Driver Comparison Test: Tesla Model Y Performance vs. Polestar 4 Dual Motor Performance Plus

## Comparison Test: Tesla Model Y Performance vs. Polestar 4 Dual Motor Performance Plus

- Brand: Car and Driver
- Model: Comparison Test: Tesla Model Y Performance vs. Polestar 4 Dual Motor Performance Plus
- Category: new_car
- Published: 2026-07-10
- Data status: Real · RSS
- Collection method: rss
- Source: [Car and Driver](https://www.caranddriver.com/reviews/comparison-test/a71772123/2026-tesla-model-y-performance-vs-2026-polestar-4-dual-motor-performance-plus-comparison-test/)
- Image URL: https://hips.hearstapps.com/hmg-prod/images/58fed26f-fc16-4606-a852-68e501ba7405.jpg

### Summary

Two automakers with a big experience gap crank up the voltage on their compact electric SUVs.

### Design keywords

- Manual review required.

### Design observations

- Manual review required.

### Design breakdown

- Proportion: Not assessed from RSS metadata. Manual source review required.
- Volume: Not assessed from RSS metadata. Manual source review required.
- Surface: Not assessed from RSS metadata. Manual source review required.
- Graphic: Not assessed from RSS metadata. Manual source review required.
- Brand language: RSS-collected source item awaiting editorial design analysis.

### Designer note

Review the linked public source before adding design interpretation or publishing this item.

## Source attribution

This card is generated from a **Real · RSS** record and must retain attribution to [Car and Driver](https://www.caranddriver.com/reviews/comparison-test/a71772123/2026-tesla-model-y-performance-vs-2026-polestar-4-dual-motor-performance-plus-comparison-test/). RSS-collected records contain feed metadata only and require editorial review before design claims are added.
